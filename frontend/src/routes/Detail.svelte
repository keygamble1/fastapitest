...
moment.locale('ko')
    ...
    let content=""
    // schema에 배열이있으면 여기서도 초기화해줘야함 자동 Null안되기때문?
    let question={answers:[],voter:[],content:''}
    let error={detail:[]}
    // svelte는 반응성시스ㅔㅁ이라서 값을 미리 넣어주지않으니까 ui에 반영이불가능

    
  
    // 누르지도않았을때 빈칸이라도 노출되게하기위해 아무것도없으면안됨
...
>
    ...
    <!-- 답변목록11 -->
     ...
     <Error error={error}/>
     <!-- 읽기전용 -->
      <!-- 부모에서 자식은 가능하나, 자식에서 부모로 값업뎃 불가 전달만가능 -->
     <form  method="post" class="my-3">
        <div class="mb-3">
            <textarea bind:value={content} rows="10" class="form-control"
